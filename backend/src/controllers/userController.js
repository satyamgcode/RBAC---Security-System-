...
export const signin = async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ id: user._id, role: user.role, permissions: user.permissions || [] }, process.env.JWT_SECRET, { expiresIn: '1d' });
...
export const updateUser = async (req, res) => {
    ...
    const { firstName, lastName, role, email, permissions } = req.body;
    ...
    try {
        ...
        if (req.user.role == 'admin') {
            ...
            user.email = email || user.email;
            user.permissions = permissions ? permissions.split(',') : user.permissions;
        }
