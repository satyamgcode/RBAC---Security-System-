...
<template>
  <nav class="bg-blue-600 p-4">
    <div class="max-w-7xl mx-auto flex justify-between items-center">
      <div class="text-white text-2xl font-semibold cursor-pointer" @click="redirectToHome">RBAC User Management</div>

      <div class="hidden md:flex space-x-4">
        <template v-if="authStore.isLoggedIn">
          <router-link to="/profile" class="text-white hover:underline">Profile</router-link>
          <router-link to="/users" class="text-white hover:underline" v-if="authStore.user?.role === 'admin' || authStore.user?.role === 'moderator'">Dashboard</router-link>
        </template>
        <template v-else>
